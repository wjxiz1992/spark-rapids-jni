...
constexpr cudf::test::debug_output_level verbosity{
...
TEST_F(FloatToStringTests, FromFloats32) {
  ...
  auto results =
      spark_rapids_jni::float_to_string(floats, cudf::get_default_stream());

  auto const expected = cudf::test::strings_column_wrapper{
      "100.0", "654321.25", "-12761.125",   "0.0", "5.0",
      "-4.0",  "NaN",       "1.2345679E11", "-0.0"};
...
TEST_F(FloatToStringTests, FromFloats64) {
  ...
  auto const expected =
      cudf::test::strings_column_wrapper{"100.0",
                                         "654321.25",
                                         "-12761.125",
                                         "1.1234567891234568",
...
TEST_F(FloatToStringTests, FromFloats32ToJsonString) {
  ...
  auto const results = spark_rapids_jni::float_to_string(
      floats,
      /*json_string=*/true, cudf::get_default_stream(),
...
TEST_F(FloatToStringTests, Float32MatchesLegacyJavaJsonFormatting) {
  auto const floats = cudf::test::fixed_width_column_wrapper<float>{
      from_bits<float>(uint32_t{0x5e1b'372a}),
  ...
  auto const expected = cudf::test::strings_column_wrapper{
      "2.79611359E18", "8.6846728E7",  "-4.6662293E12", "5.0258942E25",
      "-9.5897485E24", "9.5946444E24", "3.4028235E38"};
...
TEST_F(FloatToStringTests, Float64MatchesLegacyJavaJsonFormatting) {
  ...
  auto const expected = cudf::test::strings_column_wrapper{
      "-2.21363921575273728E17", "-5.3800104640575648E17",
      "2.5553881621949533E25",   "1.714867986910578E-234",
      "4.6116860184273879E18",   "9.223372036854776E18",
