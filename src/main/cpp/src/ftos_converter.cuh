...
constexpr unsigned int FLOAT_POW5_INV_BITCOUNT =
    (DOUBLE_POW5_INV_BITCOUNT - 64);
...
__constant__ uint32_t const POW5_INV_OFFSETS[19] = {
    0x54544554, 0x04055545, 0x10041000, 0x00400414, 0x40010000,
    0x41155555, 0x00000454, 0x00010044, 0x40000000, 0x44000041,
    0x50454450, 0x55550054, 0x51655554, 0x40004000, 0x01000001,
    0x00010500, 0x51515411, 0x05555554, 0x00000000};
...
// 9/17 digits.
template <typename T> __device__ inline uint32_t decimal_length(T const v) {
  static_assert(cuda::std::is_integral_v<T> && !cuda::std::is_signed_v<T>);
  if constexpr (sizeof(T) == sizeof(int64_t)) {
    // The average Ryu output length is 16.38 digits, so we check high-to-low.
    // Java's legacy formatter can preserve up to 19 digits for exact integral
    // doubles, even though 17 digits are sufficient for round-tripping.
    assert(v < 10'000'000'000'000'000'000ULL);
...
__device__ inline bool multipleOfPowerOf5_32(uint32_t const value,
                                             uint32_t const p) {
  return pow5factor_32(value) >= p;
}

// Returns true if value is divisible by 2^p.
__device__ inline bool multipleOfPowerOf2_32(uint32_t const value,
                                             uint32_t const p) {
...
__device__ inline uint32_t mulShift32(uint32_t const m, uint64_t const factor,
                                      int32_t const shift) {
  assert(shift > 32);
...
__device__ inline int copy_special_str(char *const result, bool const sign,
                                       bool const exponent,
                                       bool const mantissa) {
  if (mantissa) {
    memcpy(result, "NaN", 3);
...
__device__ inline int special_str_size(bool const sign, bool const exponent,
                                       bool const mantissa) {
  if (mantissa) {
    return 3;
  }
...
__device__ inline uint64_t umul128(uint64_t const a, uint64_t const b,
...
__device__ inline uint64_t shiftright128(uint64_t const lo, uint64_t const hi,
                                         uint32_t const dist) {
...
// Returns true if value is divisible by 5^p.
__device__ inline bool multipleOfPowerOf5(uint64_t const value,
                                          uint32_t const p) {
...
// Returns true if value is divisible by 2^p.
__device__ inline bool multipleOfPowerOf2(uint64_t const value,
                                          uint32_t const p) {
  assert(value != 0);
  assert(p < 64);
...
mulShift64(uint64_t const m, uint64_t const *const mul, int32_t const j) {
...
__device__ inline uint64_t
mulShiftAll64(uint64_t const m, uint64_t const *const mul, int32_t const j,
...
__device__ inline void double_computePow5(uint32_t const i,
...
// pointer.
__device__ inline void double_computeInvPow5(uint32_t const i,
                                             uint64_t *const result) {
...
__device__ inline uint32_t mulPow5InvDivPow2(uint32_t const m, uint32_t const q,
                                             int32_t const j) {
...
__device__ inline uint32_t mulPow5divPow2(uint32_t const m, uint32_t const i,
                                          int32_t const j) {
  uint64_t pow5[2];
...
__device__ inline floating_decimal_64 d2d(uint64_t const ieeeMantissa,
  ...
  if (ieeeExponent == 0) {
    ...
    e2 = static_cast<int32_t>(ieeeExponent) - DOUBLE_BIAS -
  ...
  if (e2 >= 0) {
    ...
    if (q <= 21) {
      ...
      uint32_t const mvMod5 =
          (static_cast<uint32_t>(mv)) - 5 * (static_cast<uint32_t>(div5(mv)));
      if (mvMod5 == 0) {
...
__device__ inline floating_decimal_32 f2d(uint32_t const ieeeMantissa,
                                          uint32_t const ieeeExponent) {
  int32_t e2;
  uint32_t m2;
  if (ieeeExponent == 0) {
    ...
    e2 = static_cast<int32_t>(ieeeExponent) - FLOAT_BIAS - FLOAT_MANTISSA_BITS -
         2;
  ...
  if (e2 >= 0) {
    ...
    e10 = static_cast<int32_t>(q);
    int32_t const k =
        FLOAT_POW5_INV_BITCOUNT + pow5bits(static_cast<int32_t>(q)) - 1;
    int32_t const i = -e2 + static_cast<int32_t>(q) + k;
    ...
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      ...
      // on 64-bit machines.
      int32_t const l =
          FLOAT_POW5_INV_BITCOUNT + pow5bits(static_cast<int32_t>(q - 1)) - 1;
      lastRemovedDigit = static_cast<uint8_t>(
          mulPow5InvDivPow2(mv, q - 1, -e2 + static_cast<int32_t>(q) - 1 + l) %
          10);
    }
    if (q <= 9) {
...
__device__ inline int to_chars(floating_decimal_64 const v, bool const sign,
                               char *const result) {
...
__device__ inline int to_chars(floating_decimal_32 const v, bool const sign,
                               char *const result) {
  // Step 5: Print the decimal representation.
  int index = 0;
  if (sign) {
  ...
  if (scientificNotation) {
    ...
    // Print 'E', the exponent sign, and the exponent, which has at most two
    // digits.
    result[index++] = 'E';
    if (exp < 0) {
      result[index++] = '-';
...
__device__ inline floating_decimal_64 d2d(double f, bool &ieeeSign,
                                          bool &special) {
  ...
  uint32_t const ieeeExponent = static_cast<uint32_t>(
      (bits >> DOUBLE_MANTISSA_BITS) & ((1u << DOUBLE_EXPONENT_BITS) - 1));
  // Case distinction; exit early for the easy cases.
  if (ieeeExponent == ((1u << DOUBLE_EXPONENT_BITS) - 1u) ||
      (ieeeExponent == 0 && ieeeMantissa == 0)) {
    special = true;
    return floating_decimal_64{ieeeMantissa,
                               static_cast<int32_t>(ieeeExponent)};
  }
  special = false;
  ...
  if (isSmallInt) {
    ...
    // zeros into the exponent. (This is not needed for fixed-point notation, so
    // it might be beneficial to trim trailing zeros in to_chars only if needed
    // - once fixed-point notation output is implemented.)
    for (;;) {
      uint64_t const q = div10(v.mantissa);
      uint32_t const r =
          (static_cast<uint32_t>(v.mantissa)) - 10 * (static_cast<uint32_t>(q));
      if (r != 0) {
        break;
      }
      v.mantissa = q;
...
__device__ inline floating_decimal_32 f2d(float f, bool &ieeeSign,
                                          bool &special) {
  // Step 1: Decode the floating-point number, and unify normalized and
  // subnormal cases.
  ...
  if (ieeeExponent == ((1u << FLOAT_EXPONENT_BITS) - 1u) ||
      (ieeeExponent == 0 && ieeeMantissa == 0)) {
    special = true;
    return floating_decimal_32{ieeeMantissa,
                               static_cast<int32_t>(ieeeExponent)};
  }
  special = false;
...
/** Reproduce the integral fast path in the JDK 8--17 FloatingDecimal dtoa. */
template <typename FloatType>
__device__ inline bool
legacy_java_integral_decimal(FloatType const value,
                             floating_decimal_64 *const result) {
  constexpr bool is_float = cuda::std::is_same_v<FloatType, float>;
  using bits_type = cuda::std::conditional_t<is_float, uint32_t, uint64_t>;
  constexpr int mantissa_bits =
      is_float ? FLOAT_MANTISSA_BITS : DOUBLE_MANTISSA_BITS;
  constexpr int exponent_bits =
      is_float ? FLOAT_EXPONENT_BITS : DOUBLE_EXPONENT_BITS;
  constexpr int exponent_bias = is_float ? FLOAT_BIAS : DOUBLE_BIAS;

  bits_type const bits = is_float
                             ? static_cast<bits_type>(float_to_bits(value))
                             : static_cast<bits_type>(double_to_bits(value));
  bits_type const mantissa_mask = (bits_type{1} << mantissa_bits) - 1;
  bits_type const ieee_mantissa = bits & mantissa_mask;
  uint32_t const ieee_exponent = static_cast<uint32_t>(
      (bits >> mantissa_bits) & ((bits_type{1} << exponent_bits) - 1));

  // Zero, subnormal values, and non-finite values never enter this fast path.
  if (ieee_exponent == 0 || ieee_exponent == ((1u << exponent_bits) - 1u)) {
    return false;
  }

  int32_t const binary_exponent =
      static_cast<int32_t>(ieee_exponent) - exponent_bias;
  if (binary_exponent > 62) {
    return false;
  }

  uint64_t const significand = (uint64_t{1} << mantissa_bits) | ieee_mantissa;
  int32_t const shift = binary_exponent - mantissa_bits;
  uint64_t integral_value;
  if (shift >= 0) {
    integral_value = significand << shift;
  } else {
    int32_t const right_shift = -shift;
    if (right_shift >= 64) {
      return false;
    }
    uint64_t const mask = (uint64_t{1} << right_shift) - 1;
    if ((significand & mask) != 0) {
      return false;
    }
    integral_value = significand >> right_shift;
  }

  // FloatingDecimal.developLongDigits rounds away binary precision that can
  // no longer contribute significant decimal digits before stripping zeroes.
  int32_t decimal_exponent = 0;
  if (binary_exponent > mantissa_bits + 1) {
    int32_t const insignificant_digits =
        static_cast<int32_t>(log10Pow2(binary_exponent - mantissa_bits - 2));
    if (insignificant_digits > 0) {
      uint64_t pow10 = 1;
      for (int32_t i = 0; i < insignificant_digits; ++i) {
        pow10 *= 10;
      }
      uint64_t const residue = integral_value % pow10;
      integral_value /= pow10;
      decimal_exponent += insignificant_digits;
      if (residue >= (pow10 >> 1)) {
        ++integral_value;
      }
    }
  ...
  while (integral_value % 10 == 0) {
    integral_value /= 10;
    ++decimal_exponent;
  }
  result->mantissa = integral_value;
  result->exponent = decimal_exponent;
  return true;
}

constexpr int legacy_bigint_words = 27;
constexpr int legacy_pow5_chunk = 13;
constexpr uint32_t legacy_pow5_13 = 1'220'703'125;

/**
 * Minimal fixed-capacity positive bigint used by the hard branch of the
 * legacy JDK FloatingDecimal dtoa. Binary32/64 conversion needs at most 27
 * 32-bit words, including the per-digit multiply by ten.
 */
struct legacy_big_uint {
  uint32_t words[legacy_bigint_words];
  int32_t size;

  __device__ explicit legacy_big_uint(uint64_t value = 0) : size{0} {
    if (value != 0) {
  ...
  __device__ legacy_big_uint(legacy_big_uint const &other) : size{other.size} {
    for (int32_t i = 0; i < size; ++i) {
      words[i] = other.words[i];
    }
  }

  __device__ inline void trim() {
    while (size > 0 && words[size - 1] == 0) {
      --size;
    }
  }

  __device__ inline int compare(legacy_big_uint const &other) const {
    if (size != other.size) {
  ...
  __device__ inline void multiply(uint32_t const factor) {
    if (size == 0) {
      return;
    }
    uint64_t carry = 0;
    for (int32_t i = 0; i < size; ++i) {
      uint64_t const product = static_cast<uint64_t>(words[i]) * factor + carry;
      words[i] = static_cast<uint32_t>(product);
      carry = product >> 32;
    }
    if (carry != 0) {
      assert(size < legacy_bigint_words);
      words[size++] = static_cast<uint32_t>(carry);
    }
  }

  __device__ inline void left_shift(int32_t const shift) {
    if (size == 0 || shift == 0) {
      return;
    }
    int32_t const word_shift = shift / 32;
    int32_t const bit_shift = shift % 32;
    int32_t const old_size = size;
    assert(old_size + word_shift + 1 <= legacy_bigint_words);

    if (bit_shift == 0) {
      for (int32_t i = old_size - 1; i >= 0; --i) {
        words[i + word_shift] = words[i];
      }
      size = old_size + word_shift;
    } else {
      uint32_t const top = words[old_size - 1] >> (32 - bit_shift);
      if (top != 0) {
        words[old_size + word_shift] = top;
      }
      for (int32_t i = old_size - 1; i >= 0; --i) {
        uint32_t const lower = i == 0 ? 0 : words[i - 1] >> (32 - bit_shift);
        words[i + word_shift] = (words[i] << bit_shift) | lower;
      }
      size = old_size + word_shift + (top != 0);
    }
  ...
  /** Compare this with factor * other without materializing the product. */
  __device__ inline int compare_product(legacy_big_uint const &other,
                                        uint32_t const factor) const {
    uint64_t carry = 0;
    int result = 0;
    // Carry requires a low-to-high traversal. Overwriting result is
    // intentional: the final differing word visited is the most significant
    // one.
    for (int32_t i = 0; i <= other.size; ++i) {
      uint64_t const product =
          static_cast<uint64_t>(i < other.size ? other.words[i] : 0) * factor +
          carry;
      uint32_t const product_word = static_cast<uint32_t>(product);
      uint32_t const this_word = i < size ? words[i] : 0;
      if (this_word != product_word) {
        result = this_word < product_word ? -1 : 1;
      }
      carry = product >> 32;
    }
  ...
  /** Subtract factor * other in one pass. */
  __device__ inline void subtract_product(legacy_big_uint const &other,
                                          uint32_t const factor) {
    assert(compare_product(other, factor) >= 0);
    uint64_t carry = 0;
    uint64_t borrow = 0;
    for (int32_t i = 0; i < size; ++i) {
      uint64_t const product =
          static_cast<uint64_t>(i < other.size ? other.words[i] : 0) * factor +
          carry;
      uint64_t const lhs = words[i];
      uint64_t const rhs = static_cast<uint32_t>(product) + borrow;
      words[i] = static_cast<uint32_t>(lhs - rhs);
      carry = product >> 32;
      borrow = lhs < rhs;
    }
    assert(carry == 0 && borrow == 0);
    trim();
  }

  /** Return this compared with x + y without materializing the sum. */
  __device__ inline int compare_sum(legacy_big_uint const &x,
                                    legacy_big_uint const &y) const {
    int32_t const addend_size = x.size > y.size ? x.size : y.size;
    uint64_t carry = 0;
    int result = 0;
    // Carry requires a low-to-high traversal. Overwriting result is
    // intentional: the final differing word visited is the most significant
    // one.
    for (int32_t i = 0; i <= addend_size; ++i) {
      uint64_t const sum = (i < x.size ? x.words[i] : 0) +
                           static_cast<uint64_t>(i < y.size ? y.words[i] : 0) +
                           carry;
      uint32_t const sum_word = static_cast<uint32_t>(sum);
      uint32_t const this_word = i < size ? words[i] : 0;
      if (this_word != sum_word) {
        result = this_word < sum_word ? -1 : 1;
      }
      carry = sum >> 32;
    }
    if (size > addend_size + 1) {
      return 1;
    }
    return result;
  }

  /** q = this / divisor; this = 10 * (this % divisor), where 0 <= q < 10. */
  __device__ inline int
  quotient_remainder_iteration(legacy_big_uint const &divisor) {
    assert(compare_product(divisor, 10) < 0);
    int lower = 0;
    int upper = 10;
    while (lower + 1 < upper) {
      int const candidate = (lower + upper) / 2;
      if (compare_product(divisor, candidate) >= 0) {
        lower = candidate;
      } else {
        upper = candidate;
      }
    ...
    if (lower != 0) {
      subtract_product(divisor, lower);
    }
    multiply(10);
    return lower;
  }
...
__device__ inline legacy_big_uint legacy_pow5_shift(int32_t const power5,
                                                    int32_t const power2) {
  legacy_big_uint result{1};
  int32_t remaining = power5;
  while (remaining >= legacy_pow5_chunk) {
    result.multiply(legacy_pow5_13);
    remaining -= legacy_pow5_chunk;
  }
  for (int32_t i = 0; i < remaining; ++i) {
    result.multiply(5);
  }
  result.left_shift(power2);
  return result;
}

__device__ inline legacy_big_uint
legacy_multiply_u64(legacy_big_uint const &value, uint64_t const factor) {
  uint32_t const factor_low = static_cast<uint32_t>(factor);
  uint32_t const factor_high = static_cast<uint32_t>(factor >> 32);
  legacy_big_uint result;
  uint64_t carry = 0;
  uint32_t previous_word = 0;
  for (int32_t i = 0; i <= value.size; ++i) {
    uint32_t const current_word = i < value.size ? value.words[i] : 0;
    uint64_t const low_product =
        static_cast<uint64_t>(current_word) * factor_low;
    uint64_t const high_product =
        static_cast<uint64_t>(previous_word) * factor_high;
    uint64_t const low_sum =
        static_cast<uint32_t>(low_product) +
        static_cast<uint64_t>(static_cast<uint32_t>(high_product)) + carry;
    result.words[i] = static_cast<uint32_t>(low_sum);
    carry = (low_product >> 32) + (high_product >> 32) + (low_sum >> 32);
    previous_word = current_word;
  }
  result.size = value.size + 1;
  if (carry != 0) {
    assert(result.size < legacy_bigint_words);
    result.words[result.size++] = static_cast<uint32_t>(carry);
  }
  result.trim();
  return result;
}

__device__ inline uint64_t legacy_pow5_u64(int32_t const power) {
  uint64_t result = 1;
...
__device__ inline Signed legacy_wrap_multiply(Signed const value,
                                              uint32_t const factor) {
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  return static_cast<Signed>(static_cast<unsigned_type>(value) * factor);
}

template <typename Signed>
__device__ inline Signed legacy_wrap_add(Signed const lhs, Signed const rhs) {
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  return static_cast<Signed>(static_cast<unsigned_type>(lhs) +
                             static_cast<unsigned_type>(rhs));
}

template <typename Signed>
__device__ inline Signed legacy_wrap_twice_minus(Signed const value,
                                                 Signed const subtrahend) {
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  return static_cast<Signed>((static_cast<unsigned_type>(value) << 1) -
                             static_cast<unsigned_type>(subtrahend));
}

__device__ inline void legacy_round_decimal(uint64_t *const mantissa,
                                            int32_t const digit_count,
                                            int32_t *const decimal_exponent) {
  uint64_t all_nines = 0;
  uint64_t carry_out = 1;
  for (int32_t i = 0; i < digit_count; ++i) {
    all_nines = all_nines * 10 + 9;
    carry_out *= 10;
  }
  if (*mantissa == all_nines) {
    *mantissa = carry_out / 10;
    ++*decimal_exponent;
  } else {
    ++*mantissa;
  }
...
legacy_java_small_dtoa(uint64_t const fract_bits, int32_t decimal_exponent,
                       int32_t const B5, int32_t const B2, int32_t const S5,
                       int32_t const S2, int32_t const M5, int32_t const M2,
                       floating_decimal_64 *const result) {
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  Signed b =
      static_cast<Signed>((static_cast<unsigned_type>(fract_bits) *
                           static_cast<unsigned_type>(legacy_pow5_u64(B5)))
                          << B2);
  Signed const s = static_cast<Signed>(
      static_cast<unsigned_type>(legacy_pow5_u64(S5)) << S2);
  Signed m = static_cast<Signed>(static_cast<unsigned_type>(legacy_pow5_u64(M5))
                                 << M2);
  Signed const tens = legacy_wrap_multiply(s, 10);

  uint64_t mantissa = 0;
  int32_t digit_count = 0;
  int q = b / s;
  b = legacy_wrap_multiply(static_cast<Signed>(b % s), 10);
  m = legacy_wrap_multiply(m, 10);
  bool low = b < m;
  bool high = legacy_wrap_add(b, m) > tens;
  if (q == 0 && !high) {
    --decimal_exponent;
  } else {
    mantissa = q;
  ...
  if (high) {
    Signed const difference = legacy_wrap_twice_minus(b, tens);
    if (!low || difference > 0 || (difference == 0 && (mantissa & 1) != 0)) {
      legacy_round_decimal(&mantissa, digit_count, &decimal_exponent);
    }
  }
  result->mantissa = mantissa;
  result->exponent = decimal_exponent + 1 - digit_count;
}

__device__ __noinline__ void
legacy_java_big_dtoa(uint64_t const fract_bits, int32_t decimal_exponent,
                     int32_t const B5, int32_t const B2, int32_t const S5,
                     int32_t const S2, int32_t const M2,
                     floating_decimal_64 *const result) {
  legacy_big_uint M = legacy_pow5_shift(B5, 0);
  legacy_big_uint B = legacy_multiply_u64(M, fract_bits);
  B.left_shift(B2);
  legacy_big_uint const S = legacy_pow5_shift(S5, S2);
  M.multiply(5);
  M.left_shift(M2 + 1);
  legacy_big_uint tenS{S};
  tenS.multiply(10);
...
/** Match FloatingDecimal.estimateDecExp, including each Java-strict rounding
 * step. */
__device__ inline int32_t
legacy_estimate_decimal_exponent(uint64_t const fract_bits,
                                 int32_t const binary_exponent) {
  constexpr uint64_t significand_mask =
      (uint64_t{1} << DOUBLE_MANTISSA_BITS) - 1;
  constexpr uint64_t exponent_one = static_cast<uint64_t>(DOUBLE_BIAS)
                                    << DOUBLE_MANTISSA_BITS;
  double d2;
  uint64_t const d2_bits = exponent_one | (fract_bits & significand_mask);
  memcpy(&d2, &d2_bits, sizeof(d2));

  double const fraction = __dsub_rn(d2, 1.5);
  double const scaled_fraction = __dmul_rn(fraction, 0.289529654);
  double const fraction_log10 = __dadd_rn(scaled_fraction, 0.176091259);
  double const exponent_log10 =
      __dmul_rn(static_cast<double>(binary_exponent), 0.301029995663981);
  double const estimate = __dadd_rn(fraction_log10, exponent_log10);

  uint64_t const estimate_bits = double_to_bits(estimate);
  int32_t const exponent =
      static_cast<int32_t>((estimate_bits >> DOUBLE_MANTISSA_BITS) &
                           ((1u << DOUBLE_EXPONENT_BITS) - 1u)) -
      DOUBLE_BIAS;
  bool const is_negative = (estimate_bits >> 63) != 0;
  if (exponent >= 0 && exponent < DOUBLE_MANTISSA_BITS) {
    uint64_t const mask = significand_mask >> exponent;
    int32_t const truncated =
        static_cast<int32_t>(((estimate_bits & significand_mask) |
                              (uint64_t{1} << DOUBLE_MANTISSA_BITS)) >>
                             (DOUBLE_MANTISSA_BITS - exponent));
    return is_negative
               ? ((mask & estimate_bits) == 0 ? -truncated : -truncated - 1)
               : truncated;
  }
  if (exponent < 0) {
    return (estimate_bits & ~(uint64_t{1} << 63)) == 0 ? 0
                                                       : (is_negative ? -1 : 0);
  }
...
/**
 * Convert a finite nonzero binary32/64 value with the JDK 8--17
 * FloatingDecimal stopping and rounding rules. Spark/Jackson use that legacy
 * formatting contract on the supported Java runtimes, while Ryu intentionally
 * chooses a different valid shortest representation for some values.
 */
template <typename FloatType>
__device__ inline void legacy_java_decimal(FloatType const value,
                                           floating_decimal_64 *const result) {
  if (legacy_java_integral_decimal(value, result)) {
    return;
  }
  ...
  bits_type const bits = is_float
                             ? static_cast<bits_type>(float_to_bits(value))
                             : static_cast<bits_type>(double_to_bits(value));
  bits_type fraction = bits & ((bits_type{1} << mantissa_bits) - 1);
  uint32_t const ieee_exponent = static_cast<uint32_t>(
      (bits >> mantissa_bits) & ((bits_type{1} << exponent_bits) - 1));
  ...
  if (ieee_exponent == 0) {
    significant_bits = is_float ? 32 - __clz(static_cast<uint32_t>(fraction))
                                : 64 - __clzll(static_cast<uint64_t>(fraction));
    int32_t const shift = mantissa_bits - (significant_bits - 1);
    fraction <<= shift;
    binary_exponent = 1 - shift - exponent_bias;
  } else {
    fraction |= bits_type{1} << mantissa_bits;
    significant_bits = mantissa_bits + 1;
    binary_exponent = static_cast<int32_t>(ieee_exponent) - exponent_bias;
  }

  uint64_t fract_bits = static_cast<uint64_t>(fraction);
  if constexpr (is_float) {
    fract_bits <<= DOUBLE_MANTISSA_BITS - FLOAT_MANTISSA_BITS;
  }
  int32_t const tail_zeros = __ffsll(static_cast<long long>(fract_bits)) - 1;
  int32_t const fraction_bits = DOUBLE_MANTISSA_BITS + 1 - tail_zeros;
  int32_t const tiny_bits = fraction_bits - binary_exponent - 1 > 0
                                ? fraction_bits - binary_exponent - 1
                                : 0;

  int32_t decimal_exponent =
      legacy_estimate_decimal_exponent(fract_bits, binary_exponent);
  int32_t B5 = decimal_exponent < 0 ? -decimal_exponent : 0;
  int32_t B2 = B5 + tiny_bits + binary_exponent;
  int32_t S5 = decimal_exponent > 0 ? decimal_exponent : 0;
  int32_t S2 = S5 + tiny_bits;
  int32_t const M5 = B5;
  int32_t M2 = B2 - significant_bits;

  fract_bits >>= tail_zeros;
  B2 -= fraction_bits - 1;
  int32_t const common2 = B2 < S2 ? B2 : S2;
  B2 -= common2;
  S2 -= common2;
  M2 -= common2;
  if (fraction_bits == 1) {
  ...
  // FloatingDecimal's N_5_BITS[0] entry is zero (rather than bitLength(1)).
  // This matters at the exact 32/64-bit fast-path boundaries.
  int32_t const B5_bits = B5 == 0 ? 0 : (B5 < 27 ? pow5bits(B5) : B5 * 3);
  int32_t const S5_plus_one_bits =
      S5 + 1 < 27 ? pow5bits(S5 + 1) : (S5 + 1) * 3;
  int32_t const Bbits = fraction_bits + B2 + B5_bits;
  int32_t const tenSbits = S2 + 1 + S5_plus_one_bits;
  if (Bbits < 32 && tenSbits < 32) {
    legacy_java_small_dtoa<int32_t>(fract_bits, decimal_exponent, B5, B2, S5,
                                    S2, M5, M2, result);
  } else if (Bbits < 64 && tenSbits < 64) {
    legacy_java_small_dtoa<int64_t>(fract_bits, decimal_exponent, B5, B2, S5,
                                    S2, M5, M2, result);
  } else {
    legacy_java_big_dtoa(fract_bits, decimal_exponent, B5, B2, S5, S2, M2,
                         result);
  }
}

template <typename FloatType>
__device__ inline bool is_binary_integral(FloatType const value) {
  constexpr bool is_float = cuda::std::is_same_v<FloatType, float>;
  using bits_type = cuda::std::conditional_t<is_float, uint32_t, uint64_t>;
  constexpr int mantissa_bits =
      is_float ? FLOAT_MANTISSA_BITS : DOUBLE_MANTISSA_BITS;
  ...
  bits_type const bits = is_float
                             ? static_cast<bits_type>(float_to_bits(value))
                             : static_cast<bits_type>(double_to_bits(value));
  bits_type const fraction = bits & ((bits_type{1} << mantissa_bits) - 1);
  uint32_t const ieee_exponent = static_cast<uint32_t>(
      (bits >> mantissa_bits) & ((bits_type{1} << exponent_bits) - 1));
  if (ieee_exponent == ((1u << exponent_bits) - 1u)) {
    return false;
  }
  int32_t const binary_exponent =
      static_cast<int32_t>(ieee_exponent) - exponent_bias;
  ...
  if (binary_exponent >= mantissa_bits) {
    return true;
  }
  bits_type const fractional_mask =
      (bits_type{1} << (mantissa_bits - binary_exponent)) - 1;
  return (fraction & fractional_mask) == 0;
}
...
__device__ inline int copy_special_str_json(char *const result, bool const sign,
                                            bool const exponent,
                                            bool const mantissa) {
  ...
  int const length =
      copy_special_str(result + offset, sign, exponent, mantissa);
  if (quote) {
...
__device__ inline int d2s_buffered_n_json(double f, char *result) {
  if (is_binary_integral(f)) {
    bool const sign = (double_to_bits(f) >> 63) != 0;
    floating_decimal_64 legacy_v;
    legacy_java_decimal(f, &legacy_v);
    return to_chars(legacy_v, sign, result);
  }
  bool sign = false, special = false;
  floating_decimal_64 v = d2d(f, sign, special);
  if (special) {
    return copy_special_str_json(result, sign, v.exponent, v.mantissa);
  }
  return to_chars(v, sign, result);
}
...
__device__ inline int f2s_buffered_n_json(float f, char *result) {
  if (is_binary_integral(f)) {
    bool const sign = (float_to_bits(f) >> 31) != 0;
    floating_decimal_64 legacy_v;
    legacy_java_decimal(f, &legacy_v);
    return to_chars(legacy_v, sign, result);
  }
...
__device__ inline int compute_d2s_size_json(double value) {
  if (is_binary_integral(value)) {
    bool const sign = (double_to_bits(value) >> 63) != 0;
    floating_decimal_64 legacy_v;
    legacy_java_decimal(value, &legacy_v);
    return d2s_size(legacy_v, sign);
  }
  bool sign = false, special = false;
  floating_decimal_64 v = d2d(value, sign, special);
  if (special) {
    return special_str_size_json(sign, v.exponent, v.mantissa);
  }
  return d2s_size(v, sign);
}
...
__device__ inline int compute_f2s_size_json(float value) {
  if (is_binary_integral(value)) {
    bool const sign = (float_to_bits(value) >> 31) != 0;
    floating_decimal_64 legacy_v;
    legacy_java_decimal(value, &legacy_v);
    return d2s_size(legacy_v, sign);
  }
...
__device__ inline int compute_ftos_size(FloatType value) {
  static_assert(cuda::std::is_same_v<FloatType, float> ||
                cuda::std::is_same_v<FloatType, double>);
...
template <typename T>
__device__ inline T round_half_even(T const input, int const olength,
                                    int const digits) {
 ...
 */
template <typename T>
__device__ inline int to_formatted_chars(T const v, bool const sign,
                                         char *const result, int digits) {
  static_assert(cuda::std::is_same_v<T, floating_decimal_32> ||
                cuda::std::is_same_v<T, floating_decimal_64>);
  using U =
      cuda::std::conditional_t<cuda::std::is_same_v<T, floating_decimal_32>,
                               uint32_t, uint64_t>;
  int index = 0;
  if (sign) {
    result[index++] = '-';
  ...
  if (exp < 0) {
    ...
    int actual_olength =
        int(olength) < actual_round ? int(olength) : actual_round;
    ...
    for (auto i = 0; i < actual_olength; i++) {
      result[current + actual_olength - i - 1] =
          (char)('0' + rounded_output % 10);
      rounded_output /= 10;
      index++;
    }
...
__device__ inline int compute_format_float_size(double value, int digits,
                                                bool is_float) {
  bool sign = false, special = false;
  if (is_float) {
    floating_decimal_32 v = f2d(value, sign, special);
    if (special) {
      return special_format_str_size(sign, v.exponent, v.mantissa, digits);
    }
...
__device__ inline int format_float(double value, int digits, bool is_float,
                                   char *output) {
  bool sign = false, special = false;
  if (is_float) {
    floating_decimal_32 v = f2d(value, sign, special);
    if (special) {
      return copy_format_special_str(output, sign, v.exponent, v.mantissa,
                                     digits);
    }
