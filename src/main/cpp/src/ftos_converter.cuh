...
__device__ inline uint32_t decimal_length(T const v)
{
  ...
  if constexpr (sizeof(T) == sizeof(int64_t)) {
    ...
    if (v >= 1'000'000'000'000'000'000ULL) { return 19; }
    if (v >= 100'000'000'000'000'000ULL) { return 18; }
...
__device__ inline bool legacy_java_integral_decimal(FloatType const value,
                                                    floating_decimal_64* const result)
{
  ...
  constexpr int mantissa_bits = is_float ? FLOAT_MANTISSA_BITS : DOUBLE_MANTISSA_BITS;
  constexpr int exponent_bits = is_float ? FLOAT_EXPONENT_BITS : DOUBLE_EXPONENT_BITS;
  constexpr int exponent_bias = is_float ? FLOAT_BIAS : DOUBLE_BIAS;

  bits_type const bits          = is_float ? static_cast<bits_type>(float_to_bits(value))
  ...
  uint32_t const ieee_exponent =
    static_cast<uint32_t>((bits >> mantissa_bits) & ((bits_type{1} << exponent_bits) - 1));

  // Zero, subnormal values, and non-finite values never enter this fast path.
  if (ieee_exponent == 0 || ieee_exponent == ((1u << exponent_bits) - 1u)) { return false; }

  int32_t const binary_exponent = static_cast<int32_t>(ieee_exponent) - exponent_bias;
  if (binary_exponent > 62) { return false; }
  ...
  if (binary_exponent > mantissa_bits + 1) {
    ...
    if (insignificant_digits > 0) {
      ...
      if (residue >= (pow10 >> 1)) { ++integral_value; }
    }
  }
...
struct legacy_big_uint {
  ...
  __device__ inline void left_shift(int32_t const shift)
  {
    ...
    if (bit_shift == 0) {
      ...
      if (top != 0) { words[old_size + word_shift] = top; }
      for (int32_t i = old_size - 1; i >= 0; --i) {
  ...
  __device__ inline int compare_product(legacy_big_uint const& other, uint32_t const factor) const
  {
    uint64_t carry = 0;
    int result     = 0;
    ...
    for (int32_t i = 0; i <= other.size; ++i) {
      uint64_t const product =
        static_cast<uint64_t>(i < other.size ? other.words[i] : 0) * factor + carry;
      uint32_t const product_word = static_cast<uint32_t>(product);
      uint32_t const this_word    = i < size ? words[i] : 0;
      if (this_word != product_word) { result = this_word < product_word ? -1 : 1; }
      carry = product >> 32;
    }
    if (size > other.size + 1) { return 1; }
    return result;
  }

  /** Subtract factor * other in one pass. */
  __device__ inline void subtract_product(legacy_big_uint const& other, uint32_t const factor)
  {
  ...
  __device__ inline int compare_sum(legacy_big_uint const& x, legacy_big_uint const& y) const
  {
    ...
    for (int32_t i = 0; i <= addend_size; ++i) {
      uint64_t const sum =
        (i < x.size ? x.words[i] : 0) + static_cast<uint64_t>(i < y.size ? y.words[i] : 0) + carry;
      uint32_t const sum_word  = static_cast<uint32_t>(sum);
      uint32_t const this_word = i < size ? words[i] : 0;
      if (this_word != sum_word) { result = this_word < sum_word ? -1 : 1; }
      carry = sum >> 32;
    }
    if (size > addend_size + 1) { return 1; }
    return result;
  }

  /** q = this / divisor; this = 10 * (this % divisor), where 0 <= q < 10. */
  __device__ inline int quotient_remainder_iteration(legacy_big_uint const& divisor)
  {
    ...
    if (lower != 0) { subtract_product(divisor, lower); }
    multiply(10);
    return lower;
  }
};

__device__ inline legacy_big_uint legacy_pow5_shift(int32_t const power5, int32_t const power2)
{
...
__device__ inline legacy_big_uint legacy_multiply_u64(legacy_big_uint const& value,
                                                      uint64_t const factor)
{
  ...
  for (int32_t i = 0; i <= value.size; ++i) {
    ...
    uint64_t const low_product  = static_cast<uint64_t>(current_word) * factor_low;
    uint64_t const high_product = static_cast<uint64_t>(previous_word) * factor_high;
    uint64_t const low_sum      = static_cast<uint32_t>(low_product) +
...
__device__ inline Signed legacy_wrap_multiply(Signed const value, uint32_t const factor)
{
...
{
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  return static_cast<Signed>(static_cast<unsigned_type>(lhs) + static_cast<unsigned_type>(rhs));
}

template <typename Signed>
__device__ inline Signed legacy_wrap_twice_minus(Signed const value, Signed const subtrahend)
{
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
...
template <typename Signed>
__device__ inline void legacy_java_small_dtoa(uint64_t const fract_bits,
...
{
  using unsigned_type = cuda::std::make_unsigned_t<Signed>;
  Signed b            = static_cast<Signed>(
    (static_cast<unsigned_type>(fract_bits) * static_cast<unsigned_type>(legacy_pow5_u64(B5)))
    << B2);
  Signed const s    = static_cast<Signed>(static_cast<unsigned_type>(legacy_pow5_u64(S5)) << S2);
  Signed m          = static_cast<Signed>(static_cast<unsigned_type>(legacy_pow5_u64(M5)) << M2);
  ...
  if (decimal_exponent < -3 || decimal_exponent >= 8) { high = low = false; }
  while (!low && !high) {
    q = b / s;
    b = legacy_wrap_multiply(static_cast<Signed>(b % s), 10);
    m = legacy_wrap_multiply(m, 10);
...
__device__ __noinline__ void legacy_java_big_dtoa(uint64_t const fract_bits,
                                                  int32_t decimal_exponent,
                                                  int32_t const B5,
                                                  int32_t const B2,
                                                  int32_t const S5,
 ...
 * step. */
__device__ inline int32_t legacy_estimate_decimal_exponent(uint64_t const fract_bits,
                                                           int32_t const binary_exponent)
{
  constexpr uint64_t significand_mask = (uint64_t{1} << DOUBLE_MANTISSA_BITS) - 1;
  constexpr uint64_t exponent_one     = static_cast<uint64_t>(DOUBLE_BIAS) << DOUBLE_MANTISSA_BITS;
  double d2;
  ...
  double const exponent_log10  = __dmul_rn(static_cast<double>(binary_exponent), 0.301029995663981);
  double const estimate        = __dadd_rn(fraction_log10, exponent_log10);

  uint64_t const estimate_bits = double_to_bits(estimate);
  int32_t const exponent       = static_cast<int32_t>((estimate_bits >> DOUBLE_MANTISSA_BITS) &
  ...
  if (exponent >= 0 && exponent < DOUBLE_MANTISSA_BITS) {
    ...
    int32_t const truncated = static_cast<int32_t>(
      ((estimate_bits & significand_mask) | (uint64_t{1} << DOUBLE_MANTISSA_BITS)) >>
      (DOUBLE_MANTISSA_BITS - exponent));
    return is_negative ? ((mask & estimate_bits) == 0 ? -truncated : -truncated - 1) : truncated;
  }
  if (exponent < 0) {
    return (estimate_bits & ~(uint64_t{1} << 63)) == 0 ? 0 : (is_negative ? -1 : 0);
  }
 ...
 */
template <typename FloatType>
__device__ inline void legacy_java_decimal(FloatType const value, floating_decimal_64* const result)
{
  if (legacy_java_integral_decimal(value, result)) { return; }
  ...
  if constexpr (is_float) { fract_bits <<= DOUBLE_MANTISSA_BITS - FLOAT_MANTISSA_BITS; }
  ...
  int32_t const tiny_bits =
    fraction_bits - binary_exponent - 1 > 0 ? fraction_bits - binary_exponent - 1 : 0;

  int32_t decimal_exponent = legacy_estimate_decimal_exponent(fract_bits, binary_exponent);
  ...
  int32_t const S5_plus_one_bits = S5 + 1 < 27 ? pow5bits(S5 + 1) : (S5 + 1) * 3;
  int32_t const Bbits            = fraction_bits + B2 + B5_bits;
  int32_t const tenSbits         = S2 + 1 + S5_plus_one_bits;
  if (Bbits < 32 && tenSbits < 32) {
    legacy_java_small_dtoa<int32_t>(fract_bits, decimal_exponent, B5, B2, S5, S2, M5, M2, result);
  } else if (Bbits < 64 && tenSbits < 64) {
    legacy_java_small_dtoa<int64_t>(fract_bits, decimal_exponent, B5, B2, S5, S2, M5, M2, result);
  } else {
    legacy_java_big_dtoa(fract_bits, decimal_exponent, B5, B2, S5, S2, M2, result);
  }
}
...
__device__ inline bool is_binary_integral(FloatType const value)
{
  ...
  uint32_t const ieee_exponent =
    static_cast<uint32_t>((bits >> mantissa_bits) & ((bits_type{1} << exponent_bits) - 1));
  if (ieee_exponent == ((1u << exponent_bits) - 1u)) { return false; }
  int32_t const binary_exponent = static_cast<int32_t>(ieee_exponent) - exponent_bias;
  if (binary_exponent < 0) { return false; }
  if (binary_exponent >= mantissa_bits) { return true; }
  bits_type const fractional_mask = (bits_type{1} << (mantissa_bits - binary_exponent)) - 1;
