...
#   PROVENANCE_FILE: Optional JSON provenance attachment generated by ci/jni_snapshot.py
#
###
...
DEPLOY_CLASSIFIERS="$CLASSIFIERS"
if [ -n "${PROVENANCE_FILE:-}" ]; then
    if [ ! -f "$PROVENANCE_FILE" ]; then
        echo "Provenance file does not exist: $PROVENANCE_FILE" >&2
        exit 2
    fi
    CLASS_TYPES="${CLASS_TYPES},json"
    CLASS_FILES="${CLASS_FILES},${PROVENANCE_FILE}"
    DEPLOY_CLASSIFIERS="${DEPLOY_CLASSIFIERS},provenance"
fi
